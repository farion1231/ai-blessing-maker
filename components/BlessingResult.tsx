...
export default function BlessingResult({
  ...
  return (
    <div className="bg-gradient-to-br from-yellow-50 via-white to-orange-50 rounded-xl p-6 relative overflow-hidden fade-in h-full flex flex-col">
      {/* 装饰星星 */}
      <div className="absolute top-4 right-4 text-2xl animate-pulse" aria-hidden="true">✨</div>
      {/* 右下角装饰 */}
      ...
      <div className="flex-1 relative z-10 flex flex-col min-h-0">
        <div className="text-sm font-semibold text-orange-700 mb-4 text-center space-x-2 flex-shrink-0">
          <span><span aria-hidden="true">🎉</span> {options.scenario}</span>
          <span>•</span>
          <span><span aria-hidden="true">👥</span> {options.targetPerson}</span>
          <span>•</span>
          <span><span aria-hidden="true">🎨</span> {options.style}</span>
        </div>
